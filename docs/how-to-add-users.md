...
**To make this user an admin:** run in **SQL Editor**:
...
| Goal | What to do |
|------|------------|
| Anyone can self-sign-up and log in immediately | Option 1: Disable “Confirm email” in Supabase → Auth → Providers → Email. |
| You create each user and set their password | Option 2: Supabase → Authentication → Users → Add user (with “Auto Confirm User” on). |
| Give a user admin rights | Run the `update public.profiles set is_admin = true where email = '...'` SQL in Supabase SQL Editor. |
