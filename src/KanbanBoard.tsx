...
import { Job, JobStatus, ViewState, User, Checklist, Shift } from '@/core/types';
...
const KanbanJobCard = React.memo<KanbanJobCardProps>(
  function KanbanJobCard({
    ...
    // A job "has CNC" iff it has scheduled CNC machine-hours OR CNC has actually been tracked on it
    // (legacy completion stamp or any per-variant CNC-done count). Foam presence alone no longer
    // implies CNC — that's now decided per-material (the requiresCnc slider) and surfaces through the
    // CNC accordion on the job detail. Keying off hours + real tracking keeps this badge in step with
    // that accordion without rebuilding the distributed BOM for every card.
    const cncDoneCount = Object.values(job.cncDoneByVariant ?? {}).reduce(
      (a, b) => a + (Number(b) || 0),
      0
    );
    const hasCnc =
      getMachineTotalsFromJob(job).cncHours > 0 || !!job.cncCompletedAt || cncDoneCount > 0;
      ...
      >
                    ...
                    {isMoveColumnOpen ? (
                      <>
                        <button
                          ...
                          onMouseDown={(e) => e.stopPropagation()}
                          className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-muted hover:text-white"
                        >
        ...
        <div className="mb-1 space-y-0.5 text-xs">
          <p className="truncate text-white">
            <span className="text-subtle">Part# </span>
            {partNumber || '—'}
          </p>
          <p className="truncate text-white">
          ...
          <p className="truncate text-white">
            <span className="text-subtle">{qtyLabel} </span>
            {qtyDisplay}
          </p>
          <p className="truncate text-white">
            <span className="text-subtle">PO# </span>
            {job.po || '—'}
          </p>
          {job.binLocation && (
            <p className="flex items-center gap-1 truncate text-white">
              <span className="material-symbols-outlined text-[10px] text-primary">
                location_on
              </span>
              <span className="text-subtle">Bin </span>
        ...
        {jobHours > 0 && (
          <div className="mb-1.5 flex items-center justify-between gap-2">
            <span className="text-[10px] text-muted">{jobHours.toFixed(1)}h logged</span>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
        ...
        <div className="flex items-center justify-between border-t border-line/60 pt-1.5">
          <div className="flex items-center gap-1">
            {hasChecklist && (
              <span
                className={`flex items-center gap-0.5 text-[9px] ${checklistComplete ? 'text-green-400' : 'text-muted'}`}
              >
      ...
      {deleteConfirm && (
        ...
        >
          <div className="w-full max-w-sm rounded-3xl border border-line bg-app-2 p-4 shadow-2xl">
            ...
            <p className="mb-4 text-sm text-muted">This action cannot be undone.</p>
        ...
        <div className="max-h-[calc(80vh-80px)] overflow-y-auto p-3">
          {loading ? (
            ...
            <div className="py-8 text-center">
              <p className="mb-4 text-muted">No checklist exists for this status yet.</p>
              <button
