...
const QuickScanButton: React.FC<QuickScanButtonProps> = ({ onScanComplete, onError }) => {
  ...
  const startScan = async () => {
    ...
    try {
      ...
      // Request camera with multiple fallbacks
      let stream: MediaStream | null = null;

      // Try 1: Rear camera with high resolution
      try {
  ...
  const scanFrame = useCallback(() => {
    ...
    if (!video || !canvas) {
      updateDebug('Missing video or canvas ref');
    ...
    if (video.videoWidth === 0 || video.videoHeight === 0) {
      updateDebug('Waiting for video dimensions...');
      requestAnimationFrame(scanFrame);
      return;
    }
    ...
    try {
      ...
      if (!ctx) {
        updateDebug('Failed to get canvas context');
        requestAnimationFrame(scanFrame);
        return;
      }
      ...
      if (attempts % 10 === 0) {
        ...
        if (invertedCode && invertedCode.data) {
          ...
          stopScan();
          if (mountedRef.current) {
            onScanComplete(invertedCode.data);
          }
          return;
        }
      }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- stopScan stable, omit to avoid re-create
  }, [scanAttempts, updateDebug, onScanComplete]);
  ...
  const stopScan = useCallback(() => {
    ...
    if (streamRef.current) {
      streamRef.current.getTracks().forEach((track) => {
        track.stop();
        updateDebug(`Stopped track: ${track.kind}`);
      });
      ...
      {isScanning && (
        <div className="fixed inset-0 z-50 flex flex-col bg-black">
          ...
          <div className="flex items-center justify-between bg-gradient-to-b from-black to-transparent p-4">
            <div>
              <h2 className="text-lg font-bold text-white">QR Scanner</h2>
            ...
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <div className="relative">
                ...
                <div className="relative h-64 w-64 rounded-2xl border-4 border-primary/50">
                  ...
                  {/* Scanning line */}
                  <div className="absolute inset-x-0 top-0 h-1 animate-scan-line bg-primary shadow-lg shadow-primary/50"></div>
