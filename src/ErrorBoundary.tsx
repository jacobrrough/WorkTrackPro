    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="flex min-h-screen items-center justify-center bg-background-dark p-4">
          <div className="w-full max-w-lg rounded-2xl border border-red-500/20 bg-surface p-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 rounded-lg bg-primary px-4 py-3 font-medium text-on-accent transition-colors hover:bg-primary/90"
              >
                Reload App
              </button>
              <button
                onClick={() => this.setState({ hasError: false, error: null, errorInfo: null })}
                className="flex-1 rounded-lg bg-surface-3 px-4 py-3 font-medium text-white transition-colors hover:bg-slate-600"
              >
                Try Again
              </button>
            </div>
