...
function mapProfileToUser(profile: { id: string; email: string | null; name: string | null; initials: string | null; is_admin: boolean }): User {
  return {
    id: profile.id,
    email: profile.email ?? '',
...
export const authService = {
  async checkAuth(): Promise<User | null> {
    ...
    const { data: profile } = await supabase
      ...
      .single();
    if (!profile) return mapProfileToUser({
      id: session.user.id,
